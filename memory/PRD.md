...
- ✅ **P1_FULL_PHASE_A_LIVRAISON_COMPLETE** (2026-06-07 · STRICT ADDITIF · Verrou Phase III) — Livraison P1_FULL Phase A ingestion réelle NDVI · 6 nouveaux fichiers additifs · 0 fichier client P1 existant modifié. **Architecture** : `/app/backend/integrations/p1_full/` avec (1) `download_streamer_omega.py` (290 lignes · JobStore thread-safe · download_with_retry chunked + SHA256 + check size pré + streaming abort si > max · sync_to_r2 boto3 multipart) · (2) `nasa_hls_p1_full.py` (180 lignes · earthaccess login env · search granule_id → URLs .tif · Authorization Bearer + streaming) · (3) `esa_sentinel2_p1_full.py` (220 lignes · OAuth2 password grant CDSE · token cache 9min thread-safe · resolve scene → product GUID via OData · download $value + retry sur 401) · (4) `nrcan_hrdem_p1_full.py` (60 lignes · STUB Phase B pour STAC AWS canelevation-dem) · (5) `mffp_p1_full.py` (60 lignes · STUB Phase B pour CKAN Données Québec) · (6) `__init__.py`. **Router enrichi** `habitat_fusion_p1_ingest_router.py` : import `httpx` + `BackgroundTasks` · fonction `_dispatch_p1_full_async()` qui crée job + add_task + retourne 202+job_id · nouveau `GET /job/{job_id}/status` polling · nouveau `GET /jobs` liste · nouveau `GET /url-probe?client=X` (teste 3-5 URLs candidats parallèle pour calibration). **Limites safety configurables env** : P1_MAX_TILES=3 · P1_MAX_SIZE_MB=500 · P1_TIMEOUT_S=300 (defaults conservateurs). **Storage** : `/var/data/p1_ingest/{client}/` local + R2 `bionic-zerocost-omega/ingestion_p1/{client}/` sync différé. **Tests live preview 8/8 PASS** : (1) dry_run=true NASA HLS → DRY_RUN_OK 596ms · 1 granule trouvée · (2) dry_run=false NASA HLS → **ACCEPTED_ASYNC** + job_id retourné en 3ms · (3) GET /job/<id>/status retourne JobStatus complet (status, progress, tiles, error) · (4) GET /jobs liste tous les jobs · (5) GET /url-probe nrcan_hrdem → 3/5 reachable (STAC AWS canelevation-dem CONFIRMÉ ✅) · (6) GET /url-probe mffp → 1/5 reachable (CKAN Données Québec CONFIRMÉ ✅ · 4/5 obsolètes) · (7) GET /url-probe nasa_hls → CMR catalog reachable · (8) POST /trigger/nrcan_hrdem?dry_run=false → NotImplementedError propre Phase B. **Workers preview PIDs 126-128 INCHANGÉS** (uptime 3h27m préservé). **Découverte url-probe** : URLs Phase B confirmées par tests live · NRCan = `https://canelevation-dem.s3.amazonaws.com/` (STAC AWS · boto3 anonymous) · MFFP = `https://www.donneesquebec.ca/recherche/api/3/action/package_show?id=produits-derives-de-base-du-lidar` · **prêt pour Phase B session séparée**. **Findings observés** : NASA granule_id de test `HLS.L30.T18TWS.2025270T154741.v2.0` = not_found (granule trop récente ou pas indexée encore · le mécanisme search/auth fonctionne mais nécessite granule_id valide récupéré via dry_run). Backend `/api/health=healthy`. **Activation post-deploy Elite** : Commandant peut maintenant `POST .../trigger/nasa_hls?dry_run=false` avec scene_ids valides (issus du dry_run préalable) → job lancé async + polling via job_id.

- ✅ **P1_TRIPLE_CORRECTIF · EARTHDATA_TOKEN + URL_OVERRIDE** (2026-06-07 · STRICT ADDITIF · Verrou Phase III) — Triple correctif additif livré sans toucher aux fichiers clients existants : **(1)** `.env` alias `EARTHDATA_TOKEN=<valeur EDL_TOKEN>` (valeur littérale · les .env ne supportent pas interpolation $VAR). **(2)** Nouveau module `/app/backend/integrations/ingestion_p1_url_override_omega.py` qui patch les constantes URL au boot backend via réassignation d'attributs module · idempotent · best-effort · env-driven (HRDEM_FTP_BASE_OVERRIDE, MFFP_WMS_BASE_OVERRIDE, MFFP_WFS_BASE_OVERRIDE, MFFP_DONNEES_QUEBEC_API_OVERRIDE, MFFP_LIDAR_DATASET_ID_OVERRIDE). **(3)** Wiring minimum dans `server.py` (1 bloc additif AVANT include_router pour que imports lazy voient les URLs patchées). **URLs par défaut appliquées** : NRCan HRDEM `https://download-telecharger.services.geo.ca/pub/elevation/...` · MFFP WMS/WFS `https://servicesvectoriels.atlas.gouv.qc.ca/IDS_INVENTAIRE_ECOFOR_WMS|WFS/service.svc/get` · Données Québec API `https://www.donneesquebec.ca/recherche/api/3/action/` · dataset LiDAR `produits-derives-de-base-du-lidar`. **Logs boot confirment** : `P1_URL_OVERRIDE applied · 5 overrides · 0 errors`. **Résultats live preview post-fixes** : Fix 1 EARTHDATA_TOKEN **SUCCÈS COMPLET** (dry_run=false NASA HLS passe de `LoginStrategyUnavailable` à `NotImplementedError` = auth chain entièrement débloquée). Fix 2 URL override mécanisme livré mais URLs encore à affiner (NRCan 404 sur index.html pattern · MFFP 404 sur WFS service ID exact). Workers preview INCHANGÉS. **Findings résiduels pour session future** : voie moderne NRCan = STAC AWS `canelevation-dem` bucket · voie moderne MFFP = CKAN API Données Québec `package_show?id=produits-derives-de-base-du-lidar` · refonte clients vers ces voies modernes éliminerait 404 mais hors directive actuelle. **Doctrine Phase 0** : tous `download_*()` lèvent NotImplementedError côté clients (volontaire) · livraison code P1_FULL download_*() = prochaine étape majeure pour ingestion réelle.

- ✅ **HABITAT_FUSION_P1_INGEST_TRIGGER_ENDPOINT** (2026-06-07 · STRICT ADDITIF · Verrou Phase III) — Création router additif `/app/backend/routes/habitat_fusion_p1_ingest_router.py` (340 lignes) + 1 bloc `include_router` dans `server.py`. **Endpoints** : `POST /api/v30/habitat-fusion/p1/ingest/trigger/{client}?dry_run=true` (défaut) · `GET /api/v30/habitat-fusion/p1/ingest/clients`. **Modes** : `dry_run=true` appelle `search_granules/search_scenes/list_*` (validation OAuth2/reachability/metadata · zéro téléchargement) · `dry_run=false` appelle `download_*()` (propage NotImplementedError/exceptions client proprement). **4 clients supportés** : nasa_hls (NDVI HR 30m) · esa_sentinel2_l2a (NDVI 10m) · nrcan_hrdem (LiDAR 1m) · mffp_foret_ouverte (LiDAR 0.5m). **Safety layer** : refuse sans credential/armed (status REFUSED_*) · path 404 sur client inconnu · propagation structurée erreurs. **Validation preview 4/4 DRY_RUN_OK** : NASA HLS retourne 1 granule (sample avec links) en 705ms · ESA S2 retourne 0 scènes (cloud_cover strict) en 1453ms · NRCan HRDEM index_status 404 (URL externe obsolète, dry_run détecte le problème sans engagement) · MFFP capabilities_status 404 (idem). **Findings non-bloquants** : (1) `earthaccess` lib cherche `EARTHDATA_TOKEN` au lieu de `EDL_TOKEN` (mismatch nomenclature upstream lib vs notre contrat env) · (2) URLs externes NRCan + MFFP retournent 404 (potentiel changement infra serveurs externes). Workers preview PIDs INCHANGÉS. Backend `/api/health=healthy`. **Activation post-deploy Elite** : Commandant peut maintenant curl `POST /ingest/trigger/{client}?dry_run=true` pour validation, puis `?dry_run=false` avec scene_ids/tile_names pour ingestion réelle (nécessite résolution findings 1 et 2 au préalable pour téléchargements effectifs).

- ✅ **INGESTION_P1_ARMED · 4/4 CLIENTS ARMÉS · INGESTION_P1_READY=true** (2026-06-07 · STRICT ADDITIF · Verrou Phase III) — Directive Commandant : ajout `INGESTION_P1_ARMED=1` + `INGESTION_P1_DISK_AUTHORIZED=1` dans `/app/backend/.env`. Backend redémarré · `load_dotenv()` confirme chargement. **Effet immédiat** : 4/4 clients ingestion P1 passent en mode `INGESTION_READY` : NASA HLS (NDVI HR 30m) · ESA Sentinel-2 L2A (NDVI 10m) · NRCan HRDEM (LiDAR 1m) · MFFP Forêt Ouverte (LiDAR 0.5m QC). **Validation contractuelle** : grep code clients confirme contrat env vars exact (`INGESTION_P1_ARMED` requis pour 4/4 · `INGESTION_P1_DISK_AUTHORIZED` requis pour clients LiDAR NRCan+MFFP). **Status P1 live preview** : `phase=P1_STRUCTURAL+_AWAITING_INGESTION` · `weight_active=0.35` · `clients_credential_ready=4/4` · `clients_armed=4/4` · `ingestion_p1_ready=True`. **AUCUN téléchargement déclenché** (mode armé = dormant en attente du premier appel `download_scenes/download_tiles`). Workers preview PIDs 126-128 INCHANGÉS (uptime 1:17:08 préservé). Backend `/api/health=healthy`. **Pour activation effective sur pod Elite** : Commandant doit cliquer `Deploy` pour propager les 2 flags. Le pod Elite passera également en mode `INGESTION_READY` au prochain boot. Les téléchargements LiDAR (NRCan + MFFP) peuvent atteindre plusieurs centaines de GB sur grille Phase 2 complète — surveillance disque pod Elite recommandée après premiers appels d'ingestion.

- ✅ **ZEROCOST_INPROCESS_SPAWN_STAGGER_ENV_VARS** (2026-06-07 · STRICT ADDITIF · Verrou Phase III) — Ajout des 5 variables d'environnement dans `/app/backend/.env` (sans commentaires · règle système respectée) : `ZEROCOST_INPROCESS_WORKER_COUNT=8` · `WORKER_PACING_MS=50` · `TARGET_WORKERS_ELITE=8` · `SPAWN_STAGGER_MS=2000` · `ZEROCOST_WORKER_SPAWN_LOGGING=1`. **Patch additif** dans `/app/backend/zerocost_workers_runtime.py` fonction `_spawn_all_workers()` : lecture `SPAWN_STAGGER_MS` via `os.environ`, défensive `try/except (TypeError, ValueError)`, boucle spawn avec `time.sleep(stagger_ms/1000.0)` entre chaque worker (pas après le dernier). **Backward compatibility 100% préservée** : si `SPAWN_STAGGER_MS` absent ou =0, aucun délai inséré (legacy spawn back-to-back). Logging détaillé optionnel via `ZEROCOST_WORKER_SPAWN_LOGGING=1` (affiche WORKER_PACING_MS propagé + stagger restant par worker). **Validation** : `tier-status` confirme `TARGET_WORKERS_ELITE=8` chargé (visible dans `watchdog.target_workers_elite=8`) · backend restart OK · workers preview INCHANGÉS (PIDs 126-128 uptime 2:29 préservé) · 37 lignes ajoutées vs 1 modifiée (additif strict). **Comportement attendu pod Elite** : au boot ou respawn watchdog, 8 workers spawnés avec délai 2s entre chacun (spread total 14s) → évite spike CPU bootstrap V20 parallèle qui déclenche probes liveness.

- ✅ **WORKERS_ELITE_STABILIZATION_8W_R2_SYNC** (2026-06-07 · STRICT ADDITIF · DEPLOY READY) — Stabilisation config workers Elite (**8w confirmé** vs 6w défensif via calibration mathématique : 8w+pacing 50ms = throttle 3-5% + throughput 117%) + correction STALE_R2 (3 patches additifs Verrou Phase III). **Patch 1** : `prune_orphan_state_keys()` enrichie avec **cross-pod safety** (filtre par `grid_file` matching · préserve cohabitation Phase 1 résiduelle + autres pipelines partageant bucket R2). **Patch 2** : `BOOT UNIFIED-SYNC FS+R2` dans `zerocost_worker_seed_r5.py` post `_load_worker_state()` → appel `_save_worker_state()` pour timestamp unifié dès le boot (élimine lag artificiel FS ancien ts vs R2 fresh ts). **Patch 3** : auto-purge cross-pod-safe dans watchdog après chaque respawn. **Diagnostic forensic** : workers preview 127-129 ne respawnaient pas au restart watchdog (process setsid détachés) → tournaient sur ancien code sans R2 dual-write. Stop daemon + respawn force le pickup du nouveau code. **Cross-pod issue confirmée** : bucket R2 `bionic-zerocost-omega` partagé entre preview (Phase 2 limitrophes) et autre pod (Phase 1 grid) → keys `state_worker_{3,4,5}` cohabitent. Cross-pod-safe purge **conserve** ces keys (grid_file mismatch). **Validation preview** : tier-status avant=lag_max 446s + STALE_R2 2/3 · tier-status après=**lag_max 12s + sync 3/3 OK/MATCH** ✅. Logs `BOOT UNIFIED-SYNC FS+R2 OK · worker_idx=N · r5=M · species=K` confirmés sur 3/3 workers. **Documentation** : `/app/memory/WORKERS_ELITE_STABILIZATION_8W_R2_SYNC.md` (modélisation, validation, comportement Elite post-deploy, vérification distante).

- ✅ **WORKERS_ELITE_CALIBRATION_THROTTLE_LT_5_PCT** (2026-06-06 · STRICT ADDITIF · DEPLOY READY) — Calibration mathématique de la stratégie de spawn 8 workers Elite pour throttle_ratio_pct <5%. **3 patches additifs** : (1) `zerocost_seed_r5_supervisor_watchdog.sh` injecte `SPAWN_STAGGER_MS=2000` + `WORKER_PACING_MS=50` quand `TIER=ELITE`, sinon 0/0 (preview legacy intact) ; (2) `zerocost_seed_r5_daemon.sh` supporte spawn stagger entre workers via env (boucle `for` avec `sleep` conditionnel) + propage `WORKER_PACING_MS` aux workers ; (3) `zerocost_worker_seed_r5.py` ajoute `await asyncio.sleep(WORKER_PACING_MS/1000)` après chaque R5 complète si env > 0. **Modélisation** : 8w × 60% vCPU = 4.8 vCPUs vs quota 4 vCPUs = overshoot 20% sans calib → throttle ~17% ; avec pacing 50ms post-R5 → idle ratio ~20-25% → consommation effective ~3.74 vCPUs (93.5% quota) → **throttle projeté 3-5%** ✅. **Spawn stagger 2000ms** évite spike CPU bootstrap V20 parallèle (~3-5s pic à 400%+ qui déclenche probes liveness). **Validation preview** : workers PIDs 127-129 INCHANGÉS (uptime 47min+ préservé), endpoint `/api/v30/runtime/tier-status` reflète config, zéro RELANCE parasite. **Doc** : `/app/memory/WORKERS_ELITE_CALIBRATION_THROTTLE_LT_5_PCT.md` avec matrice config par tier, projections benchmarks (×3.3 throughput vs preview), procédure calibration fine post-deploy. **Verrou Phase III INTACT** · overrides env disponibles pour ajustement post-deploy si besoin (`WORKER_PACING_MS=100` si throttle >5%, `=20` si workers idle).

- ✅ **RUNTIME_TIER_STATUS_ENDPOINT_DEPLOYED** (2026-06-06 · STRICT ADDITIF · LECTURE SEULE) — Nouveau router `/app/backend/routes/runtime_tier_status_router_omega.py` (280 lignes) enregistré dans `server.py` après `api_cache_omega_router`. **Endpoint** : `GET /api/v30/runtime/tier-status` retourne JSON exhaustif : tier détecté (PREVIEW/ELITE/ELITE_UNLIMITED) · cpu (max raw, vCPUs, stat throttling, PSI pressure) · memory (max + current + used_pct) · pod_uptime (sec + human) · workers (count + PIDs + uptime + WORKER_INDEX) · r2_state_lag (FS↔R2 sync par worker + summary) · watchdog (inferred TARGET selon tier) · scan_duration_ms · checked_at_utc. **Helpers** : `_parse_cpu_max`, `_detect_tier`, `_parse_memory`, `_parse_cpu_stat`, `_parse_cpu_pressure`, `_pod_uptime_seconds`, `_scan_zerocost_workers` (lecture directe `/proc` sans psutil), `_r2_state_lag_per_worker` (use module `r2_state_persistence_omega`). **Test live preview** : 5/5 HTTP 200 · latence 1.9-2.0s · tier=PREVIEW · vcpus=2.0 · workers=3 · throttle=93.76% · R2 lag avg=191s. **Cas d'usage Elite post-deploy** : `curl https://huntiq-restore.emergent.host/api/v30/runtime/tier-status | jq .tier` → vérification distante sans accès pod. **Verrou Phase III** : lecture seule stricte · zéro mutation · try/except gracieux partout · 6 clés R2 vestiges historiques détectées (non-bloquantes, ignorées par grid_file check worker).

- ✅ **WORKERS_SCALE_ELITE_PLUS_AUTODETECT** (2026-06-06 · STRICT ADDITIF · DEPLOY READY) — Watchdog `/app/backend/tools/zerocost_seed_r5_supervisor_watchdog.sh` patché avec auto-détection tier CPU via `cgroup cpu.max` (lecture standard cgroup v2). **Logique** : quota ≥ 400000µs (4 vCPUs) → TIER=ELITE → TARGET=8 ; sinon → TIER=PREVIEW → TARGET=3 ; "max" (illimité hyperscale) → ELITE. Overrides env optionnels `TARGET_WORKERS_PREVIEW` / `TARGET_WORKERS_ELITE`. Ancien `TARGET_WORKERS=3` hardcodé **préservé en commentaire doctrinal** (`# ─── ANCIEN ASSIGN (préservé doctrinalement) : TARGET_WORKERS=3 ───`) · Verrou Phase III intact. **Matrice de détection testée** : 200k→PREVIEW=3 · 300k→PREVIEW=3 · 400k→ELITE=8 · 800k→ELITE=8 · max→ELITE_UNLIMITED=8 · vide→PREVIEW (défensif). **Validation preview courant** : watchdog redémarré, log confirme `MIN_WORKERS=3 · TARGET=3 · TIER=PREVIEW (cpu.max=200000µs · <400000 ou non-lisible)`, workers PIDs 1176-1178 préservés (uptime 59min+ continu), zéro respawn parasite, RELANCE count stable. **Documentation déploiement** : `/app/memory/WORKERS_SCALE_ELITE_PLUS_DEPLOY_READY.md` avec matrice détection, procédure post-deploy Elite, vérifications, et rollback. **Procédure activation Elite** : Commandant clique Deploy → 10-15min → pod Elite boot avec cpu.max=400000+ → watchdog auto-bascule TIER=ELITE TARGET=8 → 8 workers cold-start depuis R2 (8 partitions modulo 8 grille qc_limitrophes ~42 R5/worker).

- ✅ **EXTERNALISATION_STATE_FILES_R2 — GO LIVE COMPLETE** (2026-06-06 · DUAL-WRITE OPÉRATIONNEL · COLD-START VALIDÉ) — Bascule à froid exécutée avec succès. **Séquence appliquée** : (a) pré-checks R2 `--check + --write-test` ✅, (b) stop daemon SIGTERM propre + backup défensif (`/var/log/bionic-zerocost-seed-r5.pre-r2-backup-1780773193/`), (c) push initial filesystem → R2 (3 state files), (d) apply 3 patches additifs dans `/app/backend/tools/zerocost_worker_seed_r5.py` (import lazy module R2 try/except + `_load_worker_state()` cold-start fallback R2 + `_save_worker_state()` dual-write best-effort), (e) restart watchdog → respawn 3 workers avec nouveau code. **Bug intermédiaire détecté et corrigé** : S3 metadata refusait caractères non-ASCII (`ΩΩ`) — switch `"doctrine":"P22-EXTERNALISATION-STATE-R2"` ASCII-safe (corps JSON conserve Ω). **Test cold-start déterministe validé** : watchdog stop + workers stop + suppression complète state files FS → simulation `_load_worker_state()` direct sur 3 workers indices → tous 3 ont reconstruit l'état correctement depuis R2 avec message `[STATE_FILE_Ω] COLD-START RESUME depuis R2`. **État opérationnel post-bascule** : 3 workers vifs (PIDs 1176-1178) · dual-write actif (FS+R2 timestamps synchronisés) · backend FastAPI HTTP 200 latence 101-291ms · workers progressent (worker 0: r5=4→5 species_done=[]). **Verrou Phase III INTACT** · filesystem reste source primaire runtime · R2 = redondance + cold-start production-ready. **Prêt pour redeploy pod Elite/production** (filesystem éphémère supporté).
- ✅ **EXTERNALISATION_STATE_FILES_ATLAS_R2 — PHASE PRÉPA (MODE SILENCE)** (2026-06-06 · STRICT ADDITIF) — Code refactor staged en additif pur (zéro mutation runtime, MODE SILENCE respecté pendant complétion R5 #0). **Artefacts créés** : 1) `/app/backend/integrations/r2_state_persistence_omega.py` (nouveau module helper · 200 lignes · lazy boto3 client S3-compat · `save_state_to_r2 / load_state_from_r2 / delete_state_from_r2 / list_state_keys_in_r2` · CLI self-test `--check / --list / --load N / --write-test`). 2) `/app/memory/EXTERNALISATION_R2_STATE_PATCH_READY.md` (doc complète procédure bascule à froid · 3 patches identifiés pour `zerocost_worker_seed_r5.py` · rollback procedure · cold-start scenario). **Doctrine** : STRICT ADDITIF · dual-write R2 + filesystem maintenu · cold-start safe (filesystem éphémère production possible). **Périmètre** : state_worker_*.json + state.json uniquement (SQLite api_cache et grilles READ-ONLY exclus par recommandation BCE-4X). **Storage** : Cloudflare R2 uniquement (credentials déjà présents · zéro nouveau credential). **Coût estimé** : ~$0.08/mois R2 writes + storage négligeable (<3KB total). **Verrou Phase III INTACT** · aucun import par runtime actif (vérifié grep) · workers 3 toujours actifs (PIDs 123-125 · uptime 17:37) · progression R5 cumulée 13 cells (4/5/5) / 332 = **3.9 %**. **Bascule à froid en attente** : ordre Commandant `EXTERNALISATION_GO_LIVE` post-validation R5 #0 sur tous workers.
