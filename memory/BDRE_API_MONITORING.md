# BDRE — MONITORING DES APIs EXTERNES
...
| API-03 | Overpass (miroir 3) | https://maps.mail.ru/osm/tools/overpass/api/interpreter | POST | Aucune |
| API-04 | Overpass (miroir 4) | https://overpass.openstreetmap.ru/api/interpreter | POST | Aucune |
...
```
Strategie actuelle:
1. Lancer les 4 miroirs en parallele (ThreadPoolExecutor)
2. Prendre la premiere reponse valide
3. Annuler les autres
4. Si toutes echouent: retry (max 3 retries)
5. Timeout adaptatif: 10s (r<1km), 15s (1-3km), 25s (3-5km), 35s (>5km)
```
...
```
Strategie BDRE:
1. Consulter le registre de sante des miroirs
...
5. Scorer la reponse (couverture, completude)
6. Si score < 0.40: declencher fallback hybride
7. Logger dans le journal d'audit
```
...
**STATUT: SPECIFICATION MONITORING COMPLETE — EN ATTENTE VALIDATION STEEVE-MAX**
