"""
comportement_loader_omega.py — ORDRE N°52-R16-D-PREP · STUB
═══════════════════════════════════════════════════════════════════════════
COMMANDANT STEEVE-MAX · BCE-4X ULTIME ABSOLU · ANTI_GÉNÉRIQUE_STRICT

Stub loader pour le hook TERRITOIRE_ULTIME COMPORTEMENT.

ATTENTION : Aucune logique métier, aucune règle, aucune donnée.

Sources externes attendues (Q3-Q4 selon planning Commandant) :
  · /app/backend/data/comportement/rsf/rsf_models_v1/
  · /app/backend/data/comportement/ssf/ssf_models_v1/
  · /app/backend/data/comportement/temporal_patterns/activity_patterns.csv
═══════════════════════════════════════════════════════════════════════════
"""
...
logger = logging.getLogger("comportement_loader_omega")

HOOK_NAME = "COMPORTEMENT"
...
def is_available() -> bool:
    ...
    d = load_dictionary("comportement_rules")
    if not d:
        return False
...
def load_data() -> None:
    ...
    logger.info(
        "COMPORTEMENT_LOADER_STUB available=%s anti_generique=true",
        is_available())
    return None
