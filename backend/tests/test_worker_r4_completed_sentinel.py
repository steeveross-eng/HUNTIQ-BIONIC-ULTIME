...
@pytest.mark.asyncio
async def test_first_respawn_not_blocked_by_long_cooldown_on_fresh_boot(patch_runtime, monkeypatch):
    """R4 · COOLDOWN_FIRST_RUN_FIX · _last_partial_respawn_at=0.0 sentinel "jamais respawné"
    → premier respawn doit s'exécuter même avec cooldown=9999s sur un pod fraîchement démarré.

    Régression : sur container court uptime (Codespace, pod redéployé),
    time.monotonic() ~ secondes ; sans le sentinel, `now - 0.0 < cooldown` →
    cooldown ACTIVE à tort → premier respawn refusé pendant 5+ minutes.
    """
    alive = patch_runtime["alive"]
    wr._pids = wr._spawn_all_workers(worker_count=8)
    crashed_pid = wr._pids[3]
    alive.discard(crashed_pid)
    assert wr._last_partial_respawn_at == 0.0, "sentinel reset attendu"
    ...
    # Premier respawn DOIT avoir eu lieu malgré cooldown=9999 (sentinel 0.0 = jamais respawné)
    assert wr._pids[3] != crashed_pid, (
        f"COOLDOWN_FIRST_RUN_FIX broken: idx 3 PID inchangé ({wr._pids[3]} == {crashed_pid}) "
        f"alors que _last_partial_respawn_at=0.0 devrait bypasser le cooldown"
    )
    assert wr._pids[3] in alive
    # Après ce respawn, _last_partial_respawn_at est désormais > 0 → cooldown s'appliquera
    assert wr._last_partial_respawn_at > 0.0
